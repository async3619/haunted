...
const searchArtistsQuery = gql<{ searchArtists: PartialDeep<Artist> }, SearchInput>`
    query ($query: String!, $limit: Int, $locale: String) {
        searchArtists(query: $query, limit: $limit, locale: $locale) {
            id
            name
        }
    }
`;
const artistsQuery = gql<{ artists: PartialDeep<Artist>[] }, GetItemsInput>`
    query ($ids: [String!]!, $locale: String) {
        artists(ids: $ids, locale: $locale) {
            id
            name
        }
    }
`;
const artistQuery = gql<{ artist: PartialDeep<Artist> }, GetItemInput>`
    query ($id: String!, $locale: String) {
        artist(id: $id, locale: $locale) {
            id
            name
        }
    }
...
const artistAlbumsQuery = gql<{ artist: { albums: ArtistAlbums } }, RootArtistAlbumsInput>`
    query ($artistId: String!, $offset: Int, $limit: Int, $locale: String) {
        artist(id: $artistId) {
...
describe("Artist (e2e)", () => {
    ...
    describe("artist (GraphQL)", () => {
        it("should be able to get artist", async () => {
            const { data } = await client.query(artistQuery, { id: "spotify::02WoRfOhF5nUVpwddshInq" }).toPromise();
        ...
        it("should be able to return null if artist is not found", async () => {
            const { data } = await client.query(artistQuery, { id: "spotify::test" }).toPromise();

            expect(data?.artist).toBeNull();
        });

        it("should respect locale", async () => {
            const result_en = await client.query(artistQuery, { id: "spotify::02WoRfOhF5nUVpwddshInq" }).toPromise();

            const result_ko = await client
                .query(artistQuery, { id: "spotify::02WoRfOhF5nUVpwddshInq", locale: "ko_KR" })
                .toPromise();
    ...
    describe("artists (GraphQL)", () => {
        it("should be able to get artists", async () => {
            const { data } = await client.query(artistsQuery, { ids: ["spotify::02WoRfOhF5nUVpwddshInq"] }).toPromise();
        ...
        it("should be able to return null if artist is not found", async () => {
            const { data } = await client.query(artistsQuery, { ids: ["spotify::test"] }).toPromise();
        ...
        it("should respect locale", async () => {
            const result_en = await client
                .query(artistsQuery, { ids: ["spotify::02WoRfOhF5nUVpwddshInq"] })
                .toPromise();

            const result_ko = await client
                .query(artistsQuery, { ids: ["spotify::02WoRfOhF5nUVpwddshInq"], locale: "ko_KR" })
                .toPromise();
    ...
    describe("searchArtists (GraphQL)", () => {
        it("should be able to search tracks", async () => {
            const { data } = await client.query(searchArtistsQuery, { query: "최엘비", limit: 1 }).toPromise();
        ...
        it("should respect limit", async () => {
            const result_1 = await client.query(searchArtistsQuery, { query: "최엘비", limit: 1 }).toPromise();

            const result_5 = await client.query(searchArtistsQuery, { query: "최엘비", limit: 5 }).toPromise();
        ...
        it("should respect locale", async () => {
            const result_en = await client.query(searchArtistsQuery, { query: "최엘비", limit: 1 }).toPromise();

            const result_ko = await client
                .query(searchArtistsQuery, { query: "최엘비", limit: 1, locale: "ko_KR" })
                .toPromise();
